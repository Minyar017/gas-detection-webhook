...
def delete_user_endpoint(req: https_fn.Request) -> https_fn.Response:
    ...
    # Vérification de l'Authorization Bearer Token
    ...
    try:
        # Vérifier et décoder le token Firebase ID
        decoded_token = auth.verify_id_token(id_token)
        uid = decoded_token['uid']
        print(f"Utilisateur authentifié: {uid}")

        # Vérifier que l'utilisateur est admin dans Firestore
        db = firestore.client()
    ...
    try:
        ...
        # Supprimer utilisateur de Firebase Authentication
        auth.delete_user(user_id)
        print(f"Utilisateur {user_id} supprimé de Firebase Auth")

        # Supprimer documents Firestore liés (collections users, userLogs, admins)
        ...
        return https_fn.Response(f"Utilisateur {user_id} supprimé avec succès", status=200)
